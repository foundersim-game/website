                ...
                <div className="flex flex-col gap-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="bg-indigo-50 border border-indigo-100 rounded-full px-4 py-1 flex items-center gap-2">
                            <span className="text-[0.625rem] font-black text-indigo-600 uppercase tracking-widest">{unspent} XP AVAILABLE</span>
                        </div>
                    </div>
                            ...
                            return (
                                <div key={perk.id} className={cn(
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[0.625rem] font-medium text-slate-500 leading-tight">{perk.description}</p>
                                    </div>
                                    <div className="shrink-0 flex flex-col items-end gap-2">
                                        {isUnlocked ? (
                                            <span className="bg-emerald-100 text-emerald-700 text-[0.5rem] font-black uppercase tracking-widest px-2 py-1 rounded-full">Unlocked</span>
                                        ) : (
                                            <Button
                                                disabled={!canAfford}
                                                onClick={() => onBuy(perk.id)}
                                                className={cn(
                                                    "h-8 px-3 rounded-xl text-[0.5625rem] font-black uppercase tracking-widest transition-all",
